...
import { EmailAlreadyExists, NotFound, InvalidCredential } from '../types/errors';
...
export class UserService extends BaseService {
	...
	async create(email: string, password: string): Promise<User> {
		...
		const hash = (await this.cryptService.apiPbkdf2(Buffer.from(password, 'utf8'), salt)) as string;
	...
	async login(email: string, password: string): Promise<User> {
		...
		const user = (await this.models.User.findOne({ where: { email: email } })) as User;

		if (!user) {
			throw new NotFound(email);
		}

		const hash = await this.cryptService.apiPbkdf2(Buffer.from(password, 'utf8'), user!.pass_salt);
	...
	async resetPassword(old_password: string, new_password: string, user: any) {
		...
		const old_hash = await this.cryptService.apiPbkdf2(Buffer.from(old_password, 'utf8'), user!.pass_salt);
		...
		// new password
		const salt = this.cryptService.apiSalt();
		const hash = (await this.cryptService.apiPbkdf2(Buffer.from(new_password, 'utf8'), salt)) as string;
